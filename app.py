...
CORS(app)
GOOGLE_API_KEY = "AIzaSyDQdV1TyAjk8njDZdDw0fJKsQKwvFR5IYM"
genai.configure(api_key=GOOGLE_API_KEY)
ELEVENLABS_API_KEY = "sk_6ab2466bc1b03591eaba90b91f15b8e86f3942091b058eb6"
        ...
        if extracted_skills and len(extracted_skills) > 0:
            ...
            if not response:
                prompt = (f"Given the conversation context:\n{context}\n"
                          f"Ask a basic technical interview question about {skill} that requires more than a one-word answer. but dont ask questions that are too big dont ask any question that reauires me to submit the code snippet etc..")
            else:
                prompt = (f"Given the conversation context:\n{context}\n"
...
def submit_response():
    ...
    try:
        ...
        logger.info(f"Image Data received: {'present' if image_data else 'not present'}")
...
def hr_profile():
    try:
        ...
        logger.irunnfo(f"HR profile report: {report}")
